...
function clearCurrencyFormatting($value) {
...
if (isset($_POST['botao'])) {
  ...
  $idade_minima = $_POST['idade_minima'];  // Novo campo
  $valor_ingresso =  clearCurrencyFormatting($_POST['valor_ingresso']);  // Novo campo
  $id_usuario = $_POST['id_user'];
    ...
    // Prepara a consulta SQL para inserir os dados no banco de dados
       // Prepara a consulta SQL para inserir os dados no banco de dados
       $cadastro = "INSERT INTO Eventos (nome_eventos, Descricao, DataInicio, DataFim, local_evento, idade_minima, valor_ingresso, id_user, foto_evento) VALUES (:nome_eventos, :Descricao, :DataInicio, :DataFim, :local_evento, :idade_minima, :valor_ingresso, :id_user, :foto_evento)";
       ...
       try {
           ...
           $result->bindParam(':idade_minima', $idade_minima, PDO::PARAM_INT);  // Novo campo
           $result->bindParam(':valor_ingresso', $valor_ingresso, PDO::PARAM_STR);  // Novo campo
...
<div class="content-wrapper">
            ...
            <div class="card card-primary">
              ...
              <form role="form" action="" method="post" enctype="multipart/form-data">
                <div class="card-body">
                  ...
                  <input type="number" class="form-control" name="idade_minima" id="idade_minima" required placeholder="Digite a idade mínima">
              </div>
              <div class="form-group">
                  <label for="valor_ingresso">Valor do Ingresso</label>
                  <input type="text" class="form-control" name="valor_ingresso" id="valor_ingresso" required placeholder="Digite o valor do ingresso">
              </div>
              ...
              <div class="table-responsive">
                ...
                <thead>
                    <tr>
                        ...
                        <th>Data Fim:</th>
                        <th>Local</th>
                        <th>Idade Mínima</th>  <!-- Novo campo -->
                        <th>Valor do Ingresso</th>  <!-- Novo campo -->
                ...
                ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($show->idade_minima); ?></td>  <!-- Novo campo -->
                                    <td><?php echo htmlspecialchars($show->valor_ingresso); ?></td>  <!-- Novo campo -->
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <a href="conteudo/del-evento.php?idDel=<?php echo $show->EventoID; ?>" onclick="return confirm('Deseja remover o evento?')" class="btn btn-danger" title="Remover Evento"><i class="fas fa-user-times"></i></a>
                                        </div>
                                    </td>
                                </tr>
                <?php
                            ...
                            echo '<tr><td colspan="8" class="text-center"><div class="alert alert-danger">Não há Eventos!</div></td></tr>';
                        }
                    } catch (Exception $e) {
                        echo '<tr><td colspan="8" class="text-center"><div class="alert alert-danger"><strong>ERRO DE PDO: </strong>' . htmlspecialchars($e->getMessage()) . '</div></td></tr>';
                    }
                ?>
                </tbody>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    const input = document.getElementById('valor_ingresso');

    function formatToCurrency(value) {
      return value
        .replace(/\D/g, '') // Remove tudo que não é dígito
        .replace(/(\d)(\d{2})$/, '$1,$2') // Adiciona a vírgula para os centavos
        .replace(/(?=(\d{3})+(\D))/, '.') // Adiciona o ponto separador de milhar
        .replace(/^[\D]/, ''); // Remove caracteres não dígitos no início
    }
